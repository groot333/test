...
const BOUNTY_MAX_REWARD_GEMS_VPS = 1_000_000;
...
const guardianMatchesMerchantOfferVps = (
  hero,
  offer,
) => {
  ...
  if (
    String(hero.type || "") !==
    String(offer.guardian_type || "")
  )
    return false;

  const level = Math.max(
    1,
    Math.floor(Number(hero.level || 1)),
  );
  if (offer.level_mode === "range") {
    return (
      level >= Number(offer.level_min || 1) &&
      level <= Number(offer.level_max || 200)
    );
  }
  return (
    level ===
    Number(offer.level_exact || 1)
  );
};

const merchantViewVps = (
  offer,
  merchantImage,
  acceptedClaim,
  myEmail = "",
  ...
  const viewer = normalizeMirrorEmail(myEmail);
  const soldByEmail = normalizeMirrorEmail(
    acceptedClaim?.user_email,
  );
  const soldByMe =
    !!acceptedClaim &&
    soldByEmail === viewer;
  ...
  const base = {
    status: merchantOfferStatusVps(
      offer,
      acceptedClaim,
    ),
    merchant_image:
      normalizeMerchantImageVps(
        merchantImage,
      ),
    offer: {
      id: offer.id,
    ...
    sold_by_name: acceptedClaim
      ? admin
        ? String(
            acceptedClaim.user_name ||
              "Un autre joueur",
          )
            .trim()
            .slice(0, 160)
  ...
  if (admin && acceptedClaim) {
    base.sold_by_email = soldByEmail;
    base.sold_guardian_id = String(
      acceptedClaim.guardian_id || "",
    );
    base.sold_guardian_level = Math.max(
      1,
      Number(
        acceptedClaim.guardian_level || 1,
      ),
    );
    base.sold_at =
      acceptedClaim.resolved_at ||
      acceptedClaim.claimed_at ||
      null;
  }
...
const botEmailVps = (id) =>
  `${String(id || "bot").toLowerCase()}@bots.astra.invalid`;

const botTargetKeyVps = (id) =>
  `bot:${String(id || "").toLowerCase()}`;

const seededShuffleVps = (seed, source) => {
  const out = [...source];
  for (let index = out.length - 1; index > 0; index -= 1) {
    const swap = stableRollVps(
      seed,
      0,
      index,
      `shuffle-${index}`,
...
const stableDecimalVps = (
  seed,
  min,
  max,
  salt = "",
) => {
  const low = Math.round(Number(min || 0) * 100);
  const high = Math.round(Number(max || 0) * 100);
  return (
    stableRollVps(
      seed,
      Math.min(low, high),
...
const RAID_BOT_DEFAULT_BUILDING_SIZES_VPS = {
  castle: 5,
  gold: 3,
  mana: 3,
  vault: 3,
...
const systemRaidBotsVps = () => {
  const names = [
    "Veilleur du Voile",
    "Bastion astral",
    "Citadelle des Brumes",
    "Garde lunaire",
    "Rempart des Échos",
    "Forteresse du Néant",
    "Sentinelle stellaire",
  ...
  return names.map((name, index) => {
    ...
    return {
      ...
      enabled: true,
      weight: 100,
      playerLevelMin: Math.max(1, tier * 2 - 1),
      playerLevelMax: Math.min(100, tier * 3 + 4),
      trophiesMin: tier * 40,
      trophiesMax: tier * 140 + 180,
      goldMin: 800 + tier * 350,
      goldMax: 1600 + tier * 650,
      manaMin: 650 + tier * 300,
      manaMax: 1350 + tier * 560,
      creditMinCents: 1,
      creditMaxCents: Math.min(10, 2 + tier),
      buildingCountMin: Math.min(10, 5 + tier),
      buildingCountMax: Math.min(15, 8 + tier),
      buildingLevelMin: Math.max(1, tier - 1),
      buildingLevelMax: Math.min(20, 3 + tier * 2),
      defenderCountMin: Math.min(
        4,
        Math.max(1, Math.floor(tier / 2)),
      ),
      defenderCountMax: Math.min(
        5,
        2 + Math.floor(tier / 2),
      ),
      defenderLevelMin: Math.max(1, tier * 2),
      defenderLevelMax: Math.min(100, 8 + tier * 5),
      defenderPowerMin: 0.8 + tier * 0.08,
      defenderPowerMax: 1.15 + tier * 0.14,
      buildingPool: [
        "magic",
        "archer",
        "cannon",
...
const normalizeRaidBotsVps = (config = {}) => {
  const configuredBuildingSizes = Object.fromEntries(
    Object.entries(config?.buildings || {})
      .map(([id, building]) => [
        String(id),
        clampInt(building?.size || 1, 1, 6),
      ])
      .filter(([id]) => validSlug(id)),
  );

  const seen = new Set();
  const configured = (
    Array.isArray(config?.raidBots)
      ? config.raidBots
      : []
  )
    .map((source) => {
      ...
      const bot = {
        id,
        name:
          String(source?.name || "Forteresse du Voile")
            .trim()
            .slice(0, 160) ||
          "Forteresse du Voile",
        enabled: true,
        weight: clampInt(source?.weight || 100, 1, 1000),
        playerLevelMin: clampInt(source?.playerLevelMin || 1, 1, 100),
        playerLevelMax: clampInt(source?.playerLevelMax || 10, 1, 100),
        trophiesMin: clampInt(source?.trophiesMin || 0, 0, 1_000_000),
        trophiesMax: clampInt(source?.trophiesMax || 500, 0, 1_000_000),
        goldMin: clampInt(source?.goldMin || 1000, 0, 10_000_000),
        goldMax: clampInt(source?.goldMax || 5000, 0, 10_000_000),
        manaMin: clampInt(source?.manaMin || 1000, 0, 10_000_000),
        manaMax: clampInt(source?.manaMax || 5000, 0, 10_000_000),
        creditMinCents: clampInt(source?.creditMinCents ?? 1, 0, 100_000),
        creditMaxCents: clampInt(source?.creditMaxCents ?? 5, 0, 100_000),
        buildingCountMin: clampInt(source?.buildingCountMin || 3, 3, 30),
        buildingCountMax: clampInt(source?.buildingCountMax || 12, 3, 30),
        buildingLevelMin: clampInt(source?.buildingLevelMin || 1, 1, 50),
        buildingLevelMax: clampInt(source?.buildingLevelMax || 4, 1, 50),
        defenderCountMin: clampInt(source?.defenderCountMin || 0, 0, 5),
        defenderCountMax: clampInt(source?.defenderCountMax || 3, 0, 5),
        defenderLevelMin: clampInt(source?.defenderLevelMin || 1, 1, 100),
        defenderLevelMax: clampInt(source?.defenderLevelMax || 10, 1, 100),
        defenderPowerMin: clampNumberVps(source?.defenderPowerMin || 1, 0.1, 10),
        defenderPowerMax: clampNumberVps(source?.defenderPowerMax || 1.5, 0.1, 10),
        buildingPool: [
          ...new Set(
            (
              Array.isArray(source?.buildingPool)
                ? source.buildingPool
                : []
            )
              .map(String)
              .filter(
                (id) =>
                  validSlug(id) && id !== "wall",
              ),
          ),
        ].slice(0, 120),
        buildingSizes: configuredBuildingSizes,
        defenderPool: [
          ...new Set(
            (
              Array.isArray(source?.defenderPool)
                ? source.defenderPool
                : []
            )
              .map(String)
      ...
      for (const [minKey, maxKey] of [
        ["playerLevelMin", "playerLevelMax"],
        ["trophiesMin", "trophiesMax"],
        ["goldMin", "goldMax"],
        ["manaMin", "manaMax"],
        ["creditMinCents", "creditMaxCents"],
        ["buildingCountMin", "buildingCountMax"],
        ["buildingLevelMin", "buildingLevelMax"],
        ["defenderCountMin", "defenderCountMax"],
        ["defenderLevelMin", "defenderLevelMax"],
        ["defenderPowerMin", "defenderPowerMax"],
      ]) {
        if (bot[minKey] > bot[maxKey]) {
          [bot[minKey], bot[maxKey]] = [
            bot[maxKey],
            bot[minKey],
          ];
        }
      ...
      if (
        bot.buildingCountMax > 3 &&
        !bot.buildingPool.length
      ) {
        return null;
      }
      if (
        bot.defenderCountMax > 0 &&
        !bot.defenderPool.length
      ) {
        return null;
      }
      return bot;
  ...
  const ids = new Set(configured.map((bot) => bot.id));
  return [
    ...configured,
    ...systemRaidBotsVps().filter(
      (bot) => !ids.has(bot.id),
    ),
  ];
};

const raidBotBuildingSizeVps = (bot, type) =>
  clampInt(
    bot?.buildingSizes?.[type] ||
      RAID_BOT_DEFAULT_BUILDING_SIZES_VPS[type] ||
      2,
    1,
    6,
  );

const generateRaidBotVps = (bot, seed) => {
  const buildingCount = stableRollVps(
    seed,
    bot.buildingCountMin,
    bot.buildingCountMax,
    "building-count",
  );
  const pool = (
    Array.isArray(bot.buildingPool)
      ? bot.buildingPool
      : []
  ).filter((type) => type !== "wall");
  const types = ["castle", "gold", "mana"];
  for (
    let index = 3;
    index < buildingCount && pool.length;
    index += 1
  ) {
    types.push(
      pool[
  ...
  const coreBuildings = types.map(
    (type, index) => ({
      id: crypto.randomUUID(),
      type,
      x: 0,
      y: 0,
      _size: raidBotBuildingSizeVps(bot, type),
      level: Math.min(
        ["lab", "market", "barracks"].includes(type)
          ? 10
          : type === "castle"
            ? 51
            : 50,
        stableRollVps(
          seed,
          bot.buildingLevelMin,
          bot.buildingLevelMax,
          `building-level-${index}`,
        ),
      ),
      rotation: stableRollVps(
        seed,
        0,
        3,
  ...
  const ordered = coreBuildings
    .map((building, index) => ({
      building,
      order:
        building.type === "castle"
          ? -1_000_000
          : -building._size * 10_000 +
            stableRollVps(
              seed,
              0,
              9999,
  ...
  const makeRows = (width) => {
    const rows = [];
    let items = [];
    let rowWidth = 0;
    let rowHeight = 0;
    for (const building of ordered) {
      const size = building._size;
      if (
        items.length &&
        rowWidth + size > width
      ) {
  ...
  for (let width = 10; width <= 20; width += 1) {
    const candidate = makeRows(width);
    if (
      candidate.width > 20 ||
      candidate.height > 20
    ) {
    ...
    const score =
      ...
      Math.abs(
        ...
        5 +
      candidate.width * candidate.height;
    if (!packed || score < packed.score) {
      packed = { ...candidate, score };
    }
  }
  if (!packed) {
    packed = {
  ...
  let y = Math.round(25 - packed.height / 2);
  for (const row of packed.rows) {
    let x = Math.round(25 - row.width / 2);
    for (const building of row.items) {
      building.x = x;
      building.y = y;
      x += building._size;
    }
  ...
  const minX = Math.min(
    ...coreBuildings.map((building) => building.x),
  );
  const minY = Math.min(
    ...coreBuildings.map((building) => building.y),
  );
  const maxX = Math.max(
    ...coreBuildings.map(
      (building) =>
        building.x + building._size - 1,
    ),
  );
  const maxY = Math.max(
    ...coreBuildings.map(
      (building) =>
        building.y + building._size - 1,
    ),
  );
  const left = Math.max(12, minX - 2);
  const right = Math.min(38, maxX + 2);
  const top = Math.max(12, minY - 2);
  const bottom = Math.min(38, maxY + 2);

  const wallLevel = stableRollVps(
    seed,
    bot.buildingLevelMin,
    bot.buildingLevelMax,
    "wall-level",
  ...
  const walls = wallPositions
    .filter((spot) => {
      ...
      if (wallSeen.has(key)) return false;
      wallSeen.add(key);
      return true;
    })
    .map((spot) => ({
  ...
  for (const building of coreBuildings) {
    for (
      let yy = building.y;
      yy < building.y + building._size;
      yy += 1
    ) {
      for (
        let xx = building.x;
        xx < building.x + building._size;
        xx += 1
      ) {
        occupied.add(`${xx},${yy}`);
      }
  ...
  const shuffledSpots = seededShuffleVps(
    `${seed}:defender-spots`,
    defenderSpots,
  );
  const defenderCount = bot.defenderPool.length
    ? stableRollVps(
        seed,
        bot.defenderCountMin,
        bot.defenderCountMax,
        "defender-count",
      )
    : 0;
  const defenders = [];
  for (
    ...
    const spot =
      shuffledSpots[
        index %
          Math.max(1, shuffledSpots.length)
      ] || {
        x: (left + right) / 2,
        y: (top + bottom) / 2,
      };
    ...
    defenders.push({
      ...
      level: stableRollVps(
        seed,
        bot.defenderLevelMin,
        bot.defenderLevelMax,
        `defender-level-${index}`,
      ),
      multiplier: stableDecimalVps(
        seed,
        bot.defenderPowerMin,
        bot.defenderPowerMax,
        `defender-power-${index}`,
      ),
      x: clampNumberVps(spot.x, 8, 42),
      y: clampNumberVps(spot.y, 8, 42),
    });
  }
...
const publicBountyVps = (
  row,
  viewerEmail = "",
) => {
  const viewer = normalizeMirrorEmail(
    viewerEmail,
  );
  const targetEmail = normalizeMirrorEmail(
    row?.target_email,
  );
  const creatorEmail = normalizeMirrorEmail(
    row?.creator_email,
  );
  const status =
    row?.status === "claiming"
  ...
  return {
    id: String(row?.id || ""),
    target_name:
      String(row?.target_name || "Joueur ASTRAL")
        .trim()
        .slice(0, 160) || "Joueur ASTRAL",
    creator_name:
      String(row?.creator_name || "Joueur ASTRAL")
        .trim()
        .slice(0, 160) || "Joueur ASTRAL",
    reward_gems: Math.max(
      1,
      Math.floor(Number(row?.reward_gems || 1)),
    ),
    status,
    created_at:
      row?.created_at || null,
...
async function settleBountiesForRaidVps(
  client,
  raid,
  attackerSnapshot,
  attackerEmail,
  ...
  const rows = await client.query(
    `SELECT *
      ...
      FOR UPDATE`,
    [normalizeMirrorEmail(defenderEmail)],
  );

  const payouts = [];
  for (const bounty of rows.rows || []) {
    if (
      normalizeMirrorEmail(
        bounty.creator_email,
      ) === normalizeMirrorEmail(attackerEmail)
    ) {
      continue;
    }
    ...
    const claimToken =
      ...
      ":RAID:" +
      String(raid.id);
    const attacker = normalizeMirrorEmail(
      attackerEmail,
    );

    if (
      bounty.status === "claiming" &&
      (
        String(bounty.claim_token || "") !==
          claimToken ||
        normalizeMirrorEmail(
          bounty.winner_email,
        ) !== attacker ||
        String(bounty.raid_id || "") !==
          String(raid.id)
      )
    ) {
      continue;
    ...
    const reward = clampInt(
      bounty.reward_gems,
      1,
      BOUNTY_MAX_REWARD_GEMS_VPS,
    );
    ...
    await client.query(
      `UPDATE astra_bounties
          SET status = 'completed',
              claim_token = $2,
              claim_started_at =
                COALESCE(claim_started_at, $3),
              completed_at = $3,
              winner_email = $4,
              winner_name = $5,
              raid_id = $6,
...
await pool.query(`
  CREATE UNIQUE INDEX IF NOT EXISTS astra_raids_base44_id_idx
  ON astra_raids (base44_id)
...
await pool.query(`
  CREATE TABLE IF NOT EXISTS astra_bounties (
    id TEXT PRIMARY KEY,
    ...
    target_village_id TEXT NOT NULL DEFAULT '',
    ...
    raid_id TEXT NOT NULL DEFAULT '',
    destruction_pct NUMERIC NOT NULL DEFAULT 0,
    debit_event_key TEXT NOT NULL DEFAULT '',
    reward_event_key TEXT NOT NULL DEFAULT '',
    cancelled_at TIMESTAMPTZ,
    cancel_reason TEXT NOT NULL DEFAULT '',
    source_id TEXT NOT NULL DEFAULT '',
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (creator_email, request_id)
  )
`);

await pool.query(`
  CREATE INDEX IF NOT EXISTS astra_bounties_target_status_idx
  ON astra_bounties (target_email, status, created_at DESC)
`);

await pool.query(`
  CREATE INDEX IF NOT EXISTS astra_bounties_status_idx
  ON astra_bounties (status, created_at DESC)
`);

await pool.query(`
  CREATE TABLE IF NOT EXISTS astra_guardian_merchant_claims (
    offer_id TEXT PRIMARY KEY,
    user_email TEXT NOT NULL,
    user_name TEXT NOT NULL DEFAULT '',
    guardian_id TEXT NOT NULL,
    guardian_type TEXT NOT NULL,
    guardian_level INTEGER NOT NULL,
    reward_gems INTEGER NOT NULL,
...
await pool.query(`
  CREATE INDEX IF NOT EXISTS astra_guardian_merchant_claims_user_idx
  ON astra_guardian_merchant_claims (user_email, claimed_at DESC)
`);
...
await pool.query(`
  CREATE TABLE IF NOT EXISTS astra_chest_openings (
    id BIGSERIAL PRIMARY KEY,
    event_key TEXT NOT NULL UNIQUE,
    ...
    chest_type TEXT NOT NULL DEFAULT '',
    rarity TEXT NOT NULL DEFAULT '',
    source TEXT NOT NULL DEFAULT 'other',
    rewards JSONB NOT NULL DEFAULT '[]'::jsonb,
    rewards_summary TEXT NOT NULL DEFAULT '',
    guardians_obtained JSONB NOT NULL DEFAULT '[]'::jsonb,
...
await pool.query(`
  CREATE INDEX IF NOT EXISTS astra_chest_openings_user_idx
  ON astra_chest_openings (user_email, occurred_at DESC)
`);
...
app.post("/v1/village/backup-ack", { preHandler: requireAstraAuth }, async (request, reply) => {
  const email = normalizeMirrorEmail(
    request.astraUser.email,
  );
  const expectedSavedAt = String(
    request.body?.saved_at || "",
  )
    .trim()
    .slice(0, 80);
  ...
  if (!expectedSavedAt) {
    return reply.code(400).send({
      ok: false,
      error:
        "Identifiant de sauvegarde manquant.",
    });
  }
  ...
  try {
    ...
    if (
      snapshot._vps_pilot_pending_backup !==
        true ||
      String(
        snapshot._vps_pilot_saved_at || "",
      ) !== expectedSavedAt
    ) {
      await client.query("COMMIT");
      return {
    ...
    delete snapshot._vps_pilot_pending_backup;
    delete snapshot._vps_pilot_pending_action;
    delete snapshot._vps_pilot_saved_at;
...
app.post("/v1/admin/chests/history", { preHandler: requireAstraAuth }, async (request, reply) => {
  if (!isAstraAdminUser(request.astraUser)) {
    return reply.code(403).send({
      ok: false,
      error: "Accès administrateur refusé.",
  ...
  const limit = clampInt(request.body?.limit || 100, 1, 500);
  if (!targetEmail) {
    return reply.code(400).send({
      ok: false,
      error: "Adresse du joueur manquante.",
    });
  }
  ...
  return {
    ...
    chests: (history.rows || []).map((row) => ({
      id: String(row.id || ""),
      event_key: String(row.event_key || ""),
      request_id: String(row.request_id || ""),
      chest_id: String(row.chest_id || ""),
      chest_type: String(row.chest_type || row.chest_id || ""),
      rarity: String(row.rarity || ""),
      source: String(row.source || ""),
      rewards: Array.isArray(row.rewards) ? row.rewards : [],
      rewards_summary: String(row.rewards_summary || ""),
      guardians_obtained: Array.isArray(row.guardians_obtained) ? row.guardians_obtained : [],
      gems_spent: Math.max(0, Number(row.gems_spent || 0)),
      result:
...
app.get("/v1/bounties/state", { preHandler: requireAstraAuth }, async (request) => {
  const email = normalizeMirrorEmail(
    request.astraUser.email,
  );
  const [rows, village] = await Promise.all([
    pool.query(
      `SELECT *
         FROM astra_bounties
        WHERE funded_at IS NOT NULL
          AND status IN ('active', 'claiming', 'completed')
  ...
  const snapshot =
    village.rows?.[0]?.snapshot &&
    typeof village.rows[0].snapshot === "object" &&
    !Array.isArray(village.rows[0].snapshot)
      ? village.rows[0].snapshot
      : {};
  const resources = ensureSnapshotResources(
  ...
  return {
    ...
    active: visible
      .filter((row) =>
        ["active", "claiming"].includes(
          String(row.status || ""),
        ),
      )
      .slice(0, 100)
      .map((row) =>
        publicBountyVps(row, email),
      ),
    completed: visible
      .filter(
    ...
    gems_balance: Math.max(
      0,
      Number(resources.gems || 0),
    ),
    require_destruction_pct: 100,
    source: "vps",
  };
});

app.post("/v1/bounties/search", { preHandler: requireAstraAuth }, async (request, reply) => {
  ...
  const needle = normalizeSearchTextVps(
    rawQuery,
  );
  const result = await pool.query(
    `SELECT
  ...
  const matches = (result.rows || [])
    .map((row) => {
      ...
      const normalizedName =
        normalizeSearchTextVps(publicName);
      const exact =
...
app.post("/v1/bounties/create", { preHandler: requireAstraAuth }, async (request, reply) => {
  const creatorEmail = normalizeMirrorEmail(
    request.astraUser.email,
  );
  ...
  const targetVillageId = String(
    request.body?.target_village_id || "",
  )
    .trim()
    .slice(0, 180);
  const reward = Math.floor(
    Number(request.body?.reward_gems || 0),
  );

  if (
    !requestId ||
    !/^[a-z0-9_-]{8,120}$/i.test(
      requestId,
    )
  ) {
    return reply.code(400).send({
      ok: false,
      error:
        "Identifiant de création de prime invalide.",
    });
  }
  if (!targetVillageId) {
    return reply.code(400).send({
      ok: false,
      error:
        "Choisissez d’abord le joueur à mettre sous prime.",
    });
  }
  if (
    !Number.isFinite(reward) ||
    reward < 1 ||
    reward > BOUNTY_MAX_REWARD_GEMS_VPS
  ) {
    return reply.code(400).send({
      ok: false,
      error:
        `La récompense doit être comprise entre 1 et ${BOUNTY_MAX_REWARD_GEMS_VPS} gemmes.`,
    });
  }
  ...
  try {
    ...
    const targetResult = await client.query(
      `SELECT
         ...
         p.is_banned,
         v.snapshot
       FROM astra_villages v
       LEFT JOIN astra_players p
         ON p.user_email = v.user_email
       WHERE
         v.user_email = $1
         OR COALESCE(v.snapshot->>'id', '') = $1
         OR COALESCE(p.source_id, '') = $1
       LIMIT 1`,
      [targetVillageId],
    );
    ...
    if (!targetResult.rowCount) {
      ...
      return reply.code(404).send({
        ok: false,
        error:
          "Ce joueur n’a pas de village ASTRAL actif.",
      });
    }
    ...
    const targetSnapshot =
      targetRow?.snapshot &&
      typeof targetRow.snapshot === "object" &&
      !Array.isArray(targetRow.snapshot)
        ? targetRow.snapshot
        : {};

    if (
      !targetEmail ||
      targetSnapshot?.initialized !== true
    ) {
      await client.query("ROLLBACK");
      return reply.code(404).send({
        ok: false,
    ...
    if (targetEmail === creatorEmail) {
      ...
      return reply.code(400).send({
        ok: false,
        error:
          "Vous ne pouvez pas poser une prime sur votre propre village.",
      });
    }
    if (targetRow?.is_banned === true) {
      await client.query("ROLLBACK");
      return reply.code(403).send({
        ok: false,
        error:
          "Ce joueur ne peut pas recevoir de prime.",
      });
    }
    ...
    const existing = await client.query(
      `SELECT *
        ...
        LIMIT 1
        FOR UPDATE`,
      [creatorEmail, requestId],
    );

    if (
      existing.rowCount &&
      ["active", "claiming", "completed"].includes(
      ...
      const snapshot =
        creator.rows?.[0]?.snapshot &&
        typeof creator.rows[0].snapshot === "object"
          ? creator.rows[0].snapshot
          : {};
      const resources =
        ensureSnapshotResources(
          cloneSnapshot(snapshot),
        );
      await client.query("COMMIT");
      return {
        ...
        bounty: publicBountyVps(
          existing.rows[0],
          creatorEmail,
        ),
        village: snapshot,
        revision: Number(
          creator.rows?.[0]?.revision || 0,
        ),
        gems_balance: Number(
          resources.gems || 0,
        ),
        gems_revision: Number(
          resources.gems_revision || 0,
        ),
        source: "vps",
      };
    }
    ...
    if (!creatorResult.rowCount) {
      ...
      return reply.code(404).send({
        ok: false,
        error:
          "Votre village ASTRAL est introuvable.",
      });
    }
    ...
    const creatorSnapshot =
      creatorRow?.snapshot &&
      typeof creatorRow.snapshot === "object" &&
      !Array.isArray(creatorRow.snapshot)
        ? cloneSnapshot(creatorRow.snapshot)
        : {};
    if (
      creatorSnapshot?.initialized !== true
    ) {
      await client.query("ROLLBACK");
      return reply.code(409).send({
        ok: false,
        error:
          "Votre village ASTRAL doit être initialisé avant de poser une prime.",
      });
    }
    ...
    if (
      Number(creatorResources.gems || 0) <
      reward
    ) {
      await client.query("ROLLBACK");
      return reply.code(409).send({
        ok: false,
        error:
          "Vous n’avez pas assez de gemmes pour financer cette prime.",
      });
    }

    applySnapshotGemDelta(
      creatorSnapshot,
      -reward,
    );
    const now = new Date().toISOString();
    ...
    await client.query(
      `INSERT INTO astra_bounties
         (id, request_id, creator_email,
          creator_name, target_email,
          target_name, target_village_id,
          reward_gems, status, funded_at,
          created_at, debit_event_key,
          updated_at)
       VALUES
         ($1, $2, $3, $4, $5, $6, $7,
          $8, 'active', $9, $9, $10, NOW())
       ON CONFLICT (creator_email, request_id)
       DO UPDATE SET
         target_email = EXCLUDED.target_email,
         target_name = EXCLUDED.target_name,
         target_village_id =
           EXCLUDED.target_village_id,
    ...
    const nextRevision =
      Number(creatorRow.revision || 0) + 1;
    await client.query(
      `UPDATE astra_villages
          SET revision = $2,
              snapshot = $3::jsonb,
...
app.post("/v1/bounties/reserve", { preHandler: requireAstraAuth }, async (request, reply) => {
  const attackerEmail = normalizeMirrorEmail(
    request.astraUser.email,
  );
  const bountyId = String(
    request.body?.bounty_id || "",
  )
    .trim()
    .slice(0, 180);
  ...
  try {
    ...
    if (!bountyResult.rowCount) {
      ...
      return reply.code(404).send({
        ok: false,
        error:
          "Cette prime n’est plus disponible.",
      });
    }
    ...
    if (
      ...
      return reply.code(400).send({
        ok: false,
        error:
          "Vous ne pouvez pas attaquer votre propre village pour une prime.",
      });
    }
    if (
      normalizeMirrorEmail(
      ...
      return reply.code(403).send({
        ok: false,
        error:
          "Vous avez déposé cette prime : vous ne pouvez pas attaquer cette cible depuis le Chasseur de primes.",
      });
    }
    ...
    if (
      ...
      return reply.code(404).send({
        ok: false,
        error:
          "Le village ciblé par cette prime n’est pas disponible.",
      });
    }
    ...
    if (
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Ce village ne peut pas être attaqué pour le moment.",
      });
    }
    if (
      buildings.some(
    ...
    const replaySeed = stableRollVps(
      reservationToken,
      1,
      2_147_483_646,
      "replay",
    ...
    const raidSnapshot = {
      ...
      removed_obstacles: Array.isArray(
        ...
        ? targetSnapshot.removed_obstacles.slice(
            0,
            600,
          )
        : [],
    ...
    await client.query(
      `INSERT INTO astra_raids
         (id, reservation_token,
          attacker_email, attacker_name,
          defender_email, defender_name,
          defender_village_id, target_key,
          snapshot, status, gold_available,
          mana_available, started_at,
          updated_at)
       VALUES
         ($1, $2, $3, $4, $5, $6, $7,
          $8, $9::jsonb, 'reserved',
      ...
      [
        ...
        String(
          targetSnapshot?.id ||
            bounty.target_village_id ||
            targetEmail,
        ).slice(0, 180),
        `bounty:${bountyId}:${targetEmail}`,
    ...
    return {
      ...
      bounty_targeted: true,
      bounty_id: bountyId,
      bounty_reward_gems: Math.max(
        1,
        Number(bounty.reward_gems || 1),
      ),
      duration: 300,
...
app.get("/v1/admin/bounties/status", { preHandler: requireAstraAuth }, async (request, reply) => {
  if (!isAstraAdminUser(request.astraUser)) {
    return reply.code(403).send({
      ok: false,
      error:
  ...
  return {
    ...
    active: Number(
      counts.rows?.[0]?.active || 0,
    ),
    completed: Number(
      counts.rows?.[0]?.completed || 0,
    ),
    migration_ready:
      migration.rows?.[0]?.value?.ready ===
      true,
    migration_updated_at:
      migration.rows?.[0]?.updated_at ||
      null,
    source: "vps",
  };
});

app.post("/v1/admin/bounties/import", { preHandler: requireAstraAuth }, async (request, reply) => {
  if (!isAstraAdminUser(request.astraUser)) {
    return reply.code(403).send({
      ok: false,
      error:
  ...
  const rows = (
    Array.isArray(request.body?.bounties)
      ? request.body.bounties
      : []
  ).slice(0, 500);
  const final =
    request.body?.final === true;
  const client = await pool.connect();
      ...
      await client.query(
        `INSERT INTO astra_bounties
           (id, request_id, creator_email,
            ...
            claim_started_at, completed_at,
            winner_email, winner_name,
            raid_id, destruction_pct,
            debit_event_key,
            reward_event_key,
         ...
         DO UPDATE SET
           ...
           target_village_id =
             EXCLUDED.target_village_id,
           reward_gems =
             EXCLUDED.reward_gems,
           status = EXCLUDED.status,
           funded_at = EXCLUDED.funded_at,
           claim_token =
             EXCLUDED.claim_token,
           claim_started_at =
             EXCLUDED.claim_started_at,
           completed_at =
             EXCLUDED.completed_at,
           winner_email =
             EXCLUDED.winner_email,
           winner_name =
             EXCLUDED.winner_name,
           raid_id = EXCLUDED.raid_id,
           destruction_pct =
             EXCLUDED.destruction_pct,
           debit_event_key =
             EXCLUDED.debit_event_key,
           reward_event_key =
             EXCLUDED.reward_event_key,
           cancelled_at =
        ...
        [
          ...
          clampNumberVps(
            source?.destruction_pct || 0,
            0,
            100,
          ),
          String(
            source?.debit_event_key || "",
          ).slice(0, 360),
          String(
            source?.reward_event_key || "",
          ).slice(0, 360),
...
app.get("/v1/raids/history-status", { preHandler: requireAstraAuth }, async () => {
  const [migration, counts] = await Promise.all([
    pool.query(
      `SELECT value, updated_at
         FROM astra_settings
        WHERE setting_key =
          'astra_raid_history_migration'
        LIMIT 1`,
    ),
    pool.query(
      `SELECT
...
app.post(
  "/v1/admin/raids/import",
  {
    preHandler: requireAstraAuth,
  ...
  async (request, reply) => {
    ...
    const rows = (
      Array.isArray(request.body?.raids)
        ? request.body.raids
        : []
    ).slice(0, 100);
    const final =
    ...
    try {
      ...
      for (const source of rows) {
        ...
        const attackerEmail =
          normalizeMirrorEmail(
            source?.attacker_email,
          );
        const defenderEmail =
          normalizeMirrorEmail(
            source?.defender_email,
          );
        ...
        const targetKey =
          snapshot?.bounty_id
            ? `bounty:${String(
                snapshot.bounty_id,
              ).slice(0, 120)}:${defenderEmail}`
            : source?.is_revenge === true
              ? `revenge:${String(
                  source?.revenge_of ||
                    base44Id,
                ).slice(0, 180)}`
          ...
          await client.query(
            `UPDATE astra_raids
                SET base44_id =
                    ...
                    credit_gain_cents =
                      CASE
                        WHEN credit_settled = FALSE
                             AND $12 = TRUE
                          THEN $13
                        ELSE credit_gain_cents
                      END,
                    credit_loss_pct =
                      CASE
                        WHEN credit_settled = FALSE
                    ...
                    credit_winner_balance_after_cents =
                      CASE
                        WHEN credit_settled = FALSE
                             AND $12 = TRUE
                          THEN $18
                        ELSE credit_winner_balance_after_cents
                      END,
                    credit_loser_balance_after_cents =
                      CASE
                        WHEN credit_settled = FALSE
                             AND $12 = TRUE
                          THEN $19
                        ELSE credit_loser_balance_after_cents
                      END,
                    credit_settled =
                      credit_settled OR $12,
                    updated_at =
                      GREATEST(
                        updated_at,
                        COALESCE(
            ...
            [
              current.id,
              base44Id,
              source?.defender_seen === true,
              replay
                ? JSON.stringify(replay)
                : null,
              source?.revenged_at || null,
              String(
                source?.revenge_raid_id || "",
              ).slice(0, 180) || null,
              source?.is_revenge === true,
              String(
                source?.revenge_of || "",
              ...
              sourceStatus,
              resolvedAt,
              source?.settlement_started_at ||
                null,
              source?.credit_settled === true,
              clampInt(
                source?.credit_gain_cents,
                0,
                MAX_RESOURCE,
              ),
              clampInt(
                source?.credit_loss_pct,
                0,
                15,
              ),
              clampInt(
                source?.credit_lost_cents,
                0,
                MAX_RESOURCE,
              ),
              normalizeMirrorEmail(
                source?.credit_winner_email,
              ) || null,
              normalizeMirrorEmail(
                source?.credit_loser_email,
              ) || null,
              clampInt(
                source?.credit_winner_balance_after_cents,
                0,
                MAX_RESOURCE,
              ),
              clampInt(
                source?.credit_loser_balance_after_cents,
                0,
                MAX_RESOURCE,
              ),
              source?.updated_date || null,
            ],
          );
          updated += 1;
          continue;
        ...
        await client.query(
          `INSERT INTO astra_raids
             (id, base44_id, reservation_token,
              attacker_email, attacker_name,
              defender_email, defender_name,
              defender_village_id, target_key,
              snapshot, status,
              gold_available, mana_available,
              gold_stolen, mana_stolen,
              stars, destruction_pct, replay,
              defender_seen, is_revenge,
              revenge_of, revenged_at,
              revenge_raid_id,
              settlement_started_at,
              credit_gain_cents,
              credit_loss_pct,
...
app.post("/v1/raid/reserve-bot", { preHandler: requireAstraAuth }, async (request, reply) => {
  const attackerEmail = normalizeMirrorEmail(
    request.astraUser.email,
  );
  const { config: adminConfig } =
    await loadVpsAdminConfig();
  const bots = normalizeRaidBotsVps(
    adminConfig || {},
  );
  ...
  if (!bots.length) {
    return reply.code(503).send({
      ok: false,
      error:
        "Aucune base bot n’est disponible.",
    });
  }
  ...
  const fresh = bots.filter(
    (bot) => !excluded.has(botTargetKeyVps(bot.id)),
  );
  const poolBots = fresh.length ? fresh : bots;

  const weighted = poolBots
    .map((bot) => ({
      bot,
      rank:
        -Math.log(
        ...
        Math.max(1, Number(bot.weight || 1)),
    }))
    .sort((a, b) => a.rank - b.rank);

  const bot = weighted[0]?.bot;
  if (!bot) {
    return reply.code(503).send({
      ok: false,
      error:
  ...
  const raidId = crypto.randomUUID();
  const generated = generateRaidBotVps(
    bot,
    `${bot.id}:${reservationToken}`,
  );
  const replaySeed = stableRollVps(
    reservationToken,
    1,
  ...
  const defenderEmail = botEmailVps(bot.id);
  const targetKey = botTargetKeyVps(bot.id);
  const attackerName =
    String(
      request.astraUser?.name ||
  ...
  const raidSnapshot = {
    ...
    replay_seed: replaySeed,
    bot_id: bot.id,
    generation_id: reservationToken,
    buildings: generated.buildings,
    defenders: generated.defenders,
    initialized: true,
    player_level: generated.level,
    trophies: generated.trophies,
    credit_gain_cents:
      generated.creditGainCents,
    sealed_walls: true,
    vps_reserved: true,
  };
  ...
  const pilotVpsRaid = {
    ...
    defender_email: defenderEmail,
    defender_name: bot.name,
    defender_village_id: bot.id,
    target_key: targetKey,
    snapshot: raidSnapshot,
    gold_available: generated.gold,
    mana_available: generated.mana,
    started_at: startedAt,
  };
...
app.post("/v1/raid/resolve", { preHandler: requireAstraAuth }, async (request, reply) => {
  ...
  try {
    ...
    if (raidSnapshot?.target_type === "bot") {
      await client.query(
        "SELECT pg_advisory_xact_lock(hashtext($1))",
        [attackerEmail],
      );
      ...
      if (!attackerResult.rowCount) {
        await client.query("ROLLBACK");
        return reply.code(404).send({
          ok: false,
          error: "Village attaquant introuvable.",
        });
      }

      const attackerRow = attackerResult.rows[0];
      const attackerSnapshot =
        attackerRow?.snapshot &&
        typeof attackerRow.snapshot === "object" &&
        !Array.isArray(attackerRow.snapshot)
          ? cloneSnapshot(attackerRow.snapshot)
          : {};

      if (raid.status === "resolved") {
        const attackerResources =
          attackerSnapshot?.resources &&
          typeof attackerSnapshot.resources === "object" &&
          !Array.isArray(attackerSnapshot.resources)
            ? attackerSnapshot.resources
            : {};
        await client.query("COMMIT");
        const won = Number(raid.stars || 0) > 0;
        return {
          ok: true,
          duplicate: true,
          ...
          attacker_credit_delta_cents:
            raid.credit_settled === true
              ? won
                ? Math.max(
                    0,
                    Number(
                      raid.credit_gain_cents || 0,
                    ),
                  )
                : -Math.max(
                    0,
      ...
      if (raid.status !== "reserved") {
        ...
        return reply.code(409).send({
          ok: false,
          error:
            "Ce raid ne peut plus être résolu.",
        });
      }

      let destruction = clampNumberVps(
        request.body?.destruction_pct || 0,
        0,
        100,
      );
      destruction =
        Math.round(destruction * 10) / 10;
      let stars = clampInt(
        request.body?.stars || 0,
        0,
        3,
      );
      ...
      const won = stars > 0;
      const lootRatio = destruction / 100;
      const goldPool = Math.max(
        0,
        Math.floor(Number(raid.gold_available || 0)),
      );
      const manaPool = Math.max(
        0,
        Math.floor(Number(raid.mana_available || 0)),
      );
      ...
      const creditGainCents = Math.max(
        0,
        Math.round(
          Number(
            raidSnapshot?.credit_gain_cents ??
              stableRollVps(
                token,
                1,
                5,
      ...
      const walletImported =
        !!walletResult.rows?.[0]?.imported_at;

      if (migrationReady || walletImported) {
        ...
        if (won) {
          ...
          creditWinnerBalanceAfter = Math.max(
            0,
            Number(
              settlement.wallet?.balance_cents ||
                0,
            ),
          );
          creditSettled = true;
      ...
      const raidChestChance = clampNumberVps(
        adminConfig?.economy?.raidChestChance ??
          40,
        0,
        100,
      );
      ...
      const botEmail = normalizeMirrorEmail(
        raid.defender_email,
      );
      const recentTargets = (
        Array.isArray(
      ...
      const pilotResolution = {
        ...
        credit_winner_email: won
          ? attackerEmail
          : botEmail,
        credit_loser_email: won
          ? botEmail
          : attackerEmail,
      ...
      await client.query(
        `UPDATE astra_raids
            SET status = 'resolved',
                ...
                credit_gain_cents = $8,
                credit_loss_pct = $9,
                credit_lost_cents = $10,
                credit_winner_email = $11,
                credit_loser_email = $12,
                credit_settled = $13,
                credit_winner_balance_after_cents = $14,
                credit_loser_balance_after_cents = $15,
                replay = $16::jsonb,
                resolved_at = NOW(),
                updated_at = NOW()
          WHERE id = $1
        ...
        [
          ...
          creditLossPct,
          creditLostCents,
          won ? attackerEmail : botEmail,
          won ? botEmail : attackerEmail,
          creditSettled,
          creditWinnerBalanceAfter,
          creditLoserBalanceAfter,
    ...
    if (raidSnapshot?.target_type !== "player") {
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Type de raid non pris en charge.",
      });
    }
    ...
    const bountyRewards =
      destruction >= 100
        ? await settleBountiesForRaidVps(
            client,
            raid,
            attackerSnapshot,
            attackerEmail,
...
app.get("/v1/merchant/state", { preHandler: requireAstraAuth }, async (request) => {
  const myEmail = normalizeMirrorEmail(
    request.astraUser.email,
  );

  const [setting, claim] = await Promise.all([
    pool.query(
      `SELECT value
         FROM astra_settings
        WHERE setting_key =
          'astra_guardian_merchant'
        LIMIT 1`,
    ),
    pool.query(
      `SELECT *
         FROM astra_guardian_merchant_claims
  ...
  const value =
    ...
    typeof setting.rows[0].value === "object" &&
    !Array.isArray(setting.rows[0].value)
      ? setting.rows[0].value
      : {};
  const merchantImage =
    normalizeMerchantImageVps(
      value?.merchant_image,
    );
  const offer =
    normalizeMerchantOfferVps(value?.offer);
  const accepted =
    offer && claim.rows?.[0]?.offer_id === offer.id
      ? claim.rows[0]
      : null;
  ...
  return {
    ...
    ...merchantViewVps(
      ...
      accepted,
      myEmail,
      isAstraAdminUser(request.astraUser),
    ),
    source: "vps",
  };
});

app.post("/v1/merchant/sell", { preHandler: requireAstraAuth }, async (request, reply) => {
  const myEmail = normalizeMirrorEmail(
    request.astraUser.email,
  );
  const myName = String(
  ...
  const requestedOfferId = String(
    request.body?.offer_id || "",
  )
    .trim()
    .slice(0, 120);
  const guardianId = String(
    request.body?.guardian_id || "",
  )
    .trim()
    .slice(0, 64);
  ...
  if (
    !requestedOfferId ||
    !validSlug(guardianId)
  ) {
    return reply.code(400).send({
      ok: false,
      error: "Échange invalide.",
    });
  }
  ...
  try {
    ...
    if (
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Cette offre du marchand n’est plus disponible.",
        status: "stale",
      });
    }
    ...
    const accepted =
      acceptedResult.rows?.[0] || null;
    const status = merchantOfferStatusVps(
      offer,
      accepted,
    );
    ...
    if (status !== "available") {
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Le marchand n’est pas disponible actuellement.",
        ...merchantViewVps(
          offer,
          merchantImage,
          null,
    ...
    const villageRow =
      villageResult.rows[0];
    const snapshot =
      villageRow?.snapshot &&
      typeof villageRow.snapshot === "object" &&
      !Array.isArray(villageRow.snapshot)
        ? cloneSnapshot(villageRow.snapshot)
        : {};
    ...
    if (snapshot?.initialized !== true) {
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Votre village doit être synchronisé avant cet échange.",
      });
    }
    ...
    if (
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Votre solde de gemmes doit être synchronisé avant cet échange. Revenez au village puis réessayez.",
      });
    }
    ...
    if (
      !guardianMatchesMerchantOfferVps(
        hero,
        offer,
      )
    ) {
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Vous ne possédez pas exactement le gardien demandé au niveau exigé.",
      });
    }
    ...
    if (hero?.locked === true) {
      ...
      return reply.code(409).send({
        ok: false,
        error:
          "Déverrouillez ce gardien avant de le vendre au marchand.",
      });
    }

    const inventory =
      snapshotInventory(snapshot);
    const removedEquipment = Object.values(
      hero?.equipment || {},
    )
      .filter((itemId) =>
        validSlug(String(itemId || "")),
      )
      .map(String);

    for (const itemId of removedEquipment) {
      inventory[itemId] = Math.min(
        MAX_RESOURCE,
        clampInt(
          inventory[itemId],
    ...
    snapshot.inventory = inventory;
    snapshot.defenders = (
      Array.isArray(snapshot.defenders)
        ? snapshot.defenders
        : []
    ).filter(
      (entry) =>
        String(entry?.id || "") !== guardianId,
    );

    if (Array.isArray(snapshot.team_presets)) {
      snapshot.team_presets =
        snapshot.team_presets.map((preset) => ({
          ...(preset &&
          ...
          members: (
            ...
            (id) =>
              String(id || "") !== guardianId,
          ),
        }));
      snapshot.team_presets_revision =
        Math.min(
          MAX_RESOURCE,
          clampInt(
            snapshot.team_presets_revision,
      ...
      snapshot.team_presets_initialized =
        true;
    }

    const gemsBefore = clampInt(
      resources.gems,
      0,
      MAX_GEMS,
    );
    const gemsBalance = Math.min(
      MAX_GEMS,
      gemsBefore + offer.reward_gems,
    );
    const gemsRevision = Math.min(
      MAX_RESOURCE,
      clampInt(
    ...
    const awards = Array.isArray(
      snapshot.guardian_merchant_awards,
    )
      ? snapshot.guardian_merchant_awards
          .filter(
            (entry) =>
    ...
    snapshot.guardian_merchant_awards = [
      ...awards,
      {
        ...
        guardian_type: hero.type,
        guardian_level: hero.level,
        gems: offer.reward_gems,
        claimed_at: now,
      },
    ].slice(-50);
    snapshot.synced_at = now;
    ...
    const claim = {
      ...
      guardian_level: clampInt(
        hero.level,
        1,
        200,
      ),
      reward_gems: clampInt(
        offer.reward_gems,
        1,
        MAX_GEMS,
      ),
    ...
    await client.query(
      `INSERT INTO astra_guardian_merchant_claims
         (offer_id, user_email, user_name,
          guardian_id, guardian_type,
          guardian_level, reward_gems,
          status, claimed_at, resolved_at,
          source_id, updated_at)
       VALUES
         ($1,$2,$3,$4,$5,$6,$7,
          'accepted',$8,$8,'',NOW())`,
    ...
    const nextRevision =
      Number(villageRow.revision || 0) + 1;
    await client.query(
      `UPDATE astra_villages
          SET revision = $2,
              snapshot = $3::jsonb,
...
app.post("/v1/admin/merchant/sync", { preHandler: requireAstraAuth }, async (request, reply) => {
  ...
  const acceptedClaim =
    request.body?.accepted_claim &&
    typeof request.body.accepted_claim === "object" &&
    !Array.isArray(request.body.accepted_claim)
      ? request.body.accepted_claim
      : null;
  const value = {
    version: 2,
  ...
  try {
    ...
    if (
      offer &&
      acceptedClaim &&
      String(acceptedClaim?.offer_id || "") ===
        String(offer.id) &&
      String(acceptedClaim?.status || "") ===
        "accepted"
    ) {
      const userEmail = normalizeMirrorEmail(
        acceptedClaim?.user_email,
      );
      const guardianId = String(
        acceptedClaim?.guardian_id || "",
        ...
        .trim()
        .slice(0, 64);
      const guardianType = String(
        acceptedClaim?.guardian_type || "",
      )
        .trim()
        .slice(0, 64);
        ...
        await client.query(
          `INSERT INTO astra_guardian_merchant_claims
           ...
           DO UPDATE SET
             ...
             guardian_type =
               EXCLUDED.guardian_type,
             guardian_level =
               EXCLUDED.guardian_level,
             reward_gems =
               EXCLUDED.reward_gems,
             status = 'accepted',
             claimed_at =
          ...
          [
            ...
            guardianType,
            clampInt(
              acceptedClaim?.guardian_level,
              1,
              200,
            ),
            clampInt(
              acceptedClaim?.reward_gems ??
                offer.reward_gems,
              1,
              MAX_GEMS,
            ),
...
app.post("/v1/chests/open", { preHandler: requireAstraAuth }, async (request, reply) => {
  ...
  return mutateLockedVillage(request, reply, async (snapshot, revision, client) => {
    ...
    const userEmail = normalizeMirrorEmail(request.astraUser.email);
    const userName = String(
      ...
      .trim()
      .slice(0, 160);
    const rewardType = String(result.type || result.item || "").slice(0, 120);
    const rewardEntry = {
      kind: String(result.kind || "").slice(0, 40),
      type: rewardType,
      rarity: String(result.rarity || "").slice(0, 60),
      gold: Math.max(0, Number(result.receivedGold || 0)),
      mana: Math.max(0, Number(result.receivedMana || 0)),
      is_new: result.isNew === true,
      copy_count: Math.max(0, Number(result.copyCount || 0)),
      duplicate_xp: Math.max(0, Number(result.duplicateXP || 0)),
      duplicate_crystals: Math.max(0, Number(result.duplicateCrystals || 0)),
    };
    const chestRewards = [rewardEntry];
    const rewardsSummary =
      result.kind === "monster"
        ? `Gardien ${rewardType} (${String(result.rarity || "")})${result.isNew ? " — nouveau" : " — doublon"} · +${Math.max(0, Number(result.receivedGold || 0))} or · +${Math.max(0, Number(result.receivedMana || 0))} mana`
        : `Objet ${rewardType} (${String(result.rarity || "")}) · +${Math.max(0, Number(result.receivedGold || 0))} or · +${Math.max(0, Number(result.receivedMana || 0))} mana`;
    const chestEventKey = `${userEmail}|${requestId}`.slice(0, 360);

    await client.query(
      `INSERT INTO astra_chest_openings (
         event_key,
         request_id,
         user_email,
         user_name,
      ...
      [
        ...
        chestId,
        chestId,
        String(result.rarity || "").slice(0, 60),
        chestId === "starterRareChest" ? "starter" : "other",
        JSON.stringify(chestRewards),
        rewardsSummary.slice(0, 1000),
        JSON.stringify(
          result.kind === "monster" && rewardType ? [rewardType] : [],
        ),
        0,
...
app.get("/v1/rollout", { preHandler: requireAstraAuth }, async () => {
  const result = await pool.query(
    `SELECT revision, value, updated_at
       FROM astra_settings
      WHERE setting_key =
        'astra_vps_player_rollout'
      LIMIT 1`,
  );
...
app.put("/v1/admin/rollout", { preHandler: requireAstraAuth }, async (request, reply) => {
  if (!isAstraAdminUser(request.astraUser)) {
    return reply.code(403).send({
      ok: false,
      error: "Accès administrateur refusé.",
